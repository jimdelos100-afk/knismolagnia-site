...
export default async function VerificationAdminPage(){
  ...
  const {data}=await supabase.from('verification_applications')
    .select('id,user_id,real_name,note,status,admin_note,created_at')
  ...
  return <><div className="admin-head"><small>实名认证</small><h1>实名认证审核</h1><p>当前为站内人工审核，暂不连接第三方实名认证平台。</p></div><VerificationManager rows={rows}/></>
}
