...
export default async function AccountPage() {
  ...
  const [
    ...
    supabase.from('profile_change_requests').select('id,status,requested_display_name,requested_bio,requested_avatar_url,admin_note,created_at').eq('user_id', user.id).order('created_at', { ascending: false }).limit(8),
  ])

  const pendingRequest = (profileChanges || []).find((r: any) => r.status === 'pending') || null
  const avatarPaths = [profile?.avatar_url, pendingRequest?.requested_avatar_url].filter(Boolean) as string[]
  const uniqueAvatarPaths = [...new Set(avatarPaths)]
  const signedMap = new Map<string, string>()
  if (uniqueAvatarPaths.length) {
    const signed = await supabase.storage.from('avatars').createSignedUrls(uniqueAvatarPaths, 3600)
    uniqueAvatarPaths.forEach((path, i) => {
      const url = signed.data?.[i]?.signedUrl
      if (url) signedMap.set(path, url)
    })
  }
  const avatarUrl = profile?.avatar_url ? signedMap.get(profile.avatar_url) || null : null
  const pendingAvatarUrl = pendingRequest?.requested_avatar_url ? signedMap.get(pendingRequest.requested_avatar_url) || null : null
  ...
  return <><SiteHeader /><main className="dashboard wrap">
    <aside className="panel account-side">
      ...
      <span className="badge">Level {membership?.access_level ?? 2}</span>
      <p>{user.email}</p><LogoutButton />
    </aside>
    <section>
      <small>我的账号</small><h1 className="page-title">欢迎回来，<em>{profile?.display_name || '成员'} ♡</em></h1>
      {membership?.status !== 'active' && <div className="panel warning-panel">当前账号状态为受限。你仍可查看账号页，但不能进入受权限保护的 Level。</div>}
      <div className="stats">
